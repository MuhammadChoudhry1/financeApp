...
def generate_token_response(username):
    token = encode({
        'user': username
        # Removed 'exp' to make the token valid indefinitely
    ...
    if auth:
        ...
        if user:
            ...
            if bcrypt.checkpw(auth.password.encode('utf-8'), user[1].encode('utf-8')):  # Ensure password comparison is correct
                return generate_token_response(auth.username)  # Use the updated function
            else:
...
@jwt_required
def logout(username):  # <-- Accept the injected username
    try:
        token = request.headers.get('x-access-token')
        print(f"Token received for logout: {token}")  # Debugging log

        cursor.execute("INSERT INTO blacklist (token) VALUES (?)", (token,))
        conn.commit()
        print("Token added to blacklist")  # Debugging log

        return make_response(jsonify({'message': 'Successfully logged out'}), 200)
    except Exception as e:
        print(f"Error during logout: {e}")  # Debugging log
